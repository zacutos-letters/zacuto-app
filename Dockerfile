...
ARG ZACUTO_EDITION_VERSION=main

# add key
RUN  mkdir -p ~/.ssh && ssh-keyscan -t rsa github.com >> ~/.ssh/known_hosts

# Replace git URL below to point to your git repository 
RUN  git clone https://github.com/zacutos-letters/zacuto-app.git \
    # replace my-edition with name of your app
    && cd my-edition \
    && echo Checking out ${ZACUTO_EDITION_VERSION} \
    && git checkout ${ZACUTO_EDITION_VERSION} \
    && ant
...
# replace my-edition with name of your app
COPY --from=tei /tmp/zacuto-app/build/*.xar /exist/autodeploy/
