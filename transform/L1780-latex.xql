...
module namespace model="http://www.tei-c.org/pm/models/L1780/latex";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(app) return
                        latex:omit($config, ., ("tei-app", css:map-rend-to-class(.)), .)
                    ...
                    case element(idno) return
                        if (parent::person) then
                            latex:omit($config, ., ("tei-idno", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
