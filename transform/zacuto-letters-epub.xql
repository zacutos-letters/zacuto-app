            ...
            return
                            ...
                            if (@type='summary') then
                                ...
                                if (parent::person) then
                                    ...
                                    if (parent::place) then
                                        epub:block($config, ., ("tei-note4", css:map-rend-to-class(.)), .)
                                    else
                                        epub:note($config, ., ("tei-note5", css:map-rend-to-class(.)), ., @place, @n)
                    ...
                    case element(bibl) return
                        if (parent::cit and not($parameters?view='edition')) then
                            ...
                            if (parent::person) then
                                ...
                                return
                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-bibl2", css:map-rend-to-class(.)), $content)
                            else
                    ...
                    case element(date) return
                        if (parent::correspAction) then
                            ...
                            if (@when) then
                                epub:alternate($config, ., ("tei-date5", css:map-rend-to-class(.)), ., ., @when)
                            else
                                if (text()) then
                                        ...
                                        if (@source='treccani') then
                                            ...
                                            if (@source='viaf') then
                                                ...
                                                if (@source='wikidata') then
                                                    ...
                                                    return
                                                                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-idno7", css:map-rend-to-class(.)), $content)
                                                else
                                                    if (parent::person) then
                    ...
                    case element(persName) return
                        if (parent::correspAction) then
                            ...
                            if (@type='main' and @xml:lang='he' and $parameters?view='biogram') then
                                ...
                                return
                                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-persName2", css:map-rend-to-class(.)), $content)
                            else
                                if (@type='main' and @xml:lang='en' and $parameters?view='biogram') then
                                    ...
                                    return
                                                                        epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-persName3", css:map-rend-to-class(.)), $content)
                                else
                                    if (ancestor::text and $parameters?view='edition') then
                                        epub:alternate($config, ., ("tei-persName5", "persName", css:map-rend-to-class(.)), ., ., id(@key, doc("/db/apps/zacutos-letters/data/people.xml")))
                                    else
                            ...
                            if ($parameters?view='biogram') then
                                (
                                    ...
                                    if (exists(note[@type="bioSummary"] or death or birth)) then
                                        ...
                                        (),
                                    (: Birth :)
                                    epub:block($config, ., ("tei-person12", css:map-rend-to-class(.)), birth),
                                    (: Death :)
                                    epub:block($config, ., ("tei-person13", css:map-rend-to-class(.)), death),
                                    if (exists(note[@type="bioSummary"])) then
                                        epub:block($config, ., ("tei-person14", css:map-rend-to-class(.)), note[@type="bioSummary"])
                                    else
                                        (),
                                    if (exists(idno)) then
                                        ...
                                        (),
                                    if (exists(idno)) then
                                        epub:block($config, ., ("tei-person16", css:map-rend-to-class(.)), idno)
                                    else
                                        ()
                                )
                    ...
                    case element(placeName) return
                        if ($parameters?view='edition') then
                            epub:alternate($config, ., ("tei-placeName1", "placeName", css:map-rend-to-class(.)), ., ., id(@key, doc("/db/apps/zacutos-letters/data/places.xml")))
                        else
                            if (ancestor::text and $parameters?view='edition') then
                                epub:alternate($config, ., ("tei-placeName2", css:map-rend-to-class(.)), ., ., id(@key, doc("/db/apps/zacutos-letters/data/places.xml")))
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(support) return
                        ...
                        return
                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-support", css:map-rend-to-class(.)), $content)
                    ...
                    case element(extent) return
                        epub:block($config, ., ("tei-extent", css:map-rend-to-class(.)), .)
                    case element(collation) return
                        ...
                        return
                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-collation", css:map-rend-to-class(.)), $content)
                    case element(condition) return
                        ...
                        return
                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-condition", css:map-rend-to-class(.)), $content)
                    case element(foliation) return
                        ...
                        return
                                                epub:block(map:merge(($config, map:entry("template", true()))), ., ("tei-foliation", css:map-rend-to-class(.)), $content)
