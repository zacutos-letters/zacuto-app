...
module namespace model="http://www.tei-c.org/pm/models/places/fo";
            ...
            return
                    ...
                    case element(note) return
                        if (parent::place) then
                            fo:inline($config, ., ("tei-note1", css:map-rend-to-class(.)), .)
                        else
                            if (@type='summary') then
                                ...
                                if (parent::person) then
                                    ...
                                    fo:inline($config, ., ("tei-note4", css:map-rend-to-class(.)), .)
                    ...
                    case element(title) return
                        fo:heading($config, ., ("tei-title1", css:map-rend-to-class(.)), ., 2)
                    ...
                    case element(body) return
                        (
                            fo:block($config, ., ("tei-body1", css:map-rend-to-class(.)), .),
                            fo:index($config, ., ("tei-body2", css:map-rend-to-class(.)), ., 'toc')
                        )
                    ...
                    case element(birth) return
                        fo:omit($config, ., ("tei-birth", css:map-rend-to-class(.)), .)
                    case element(death) return
                        fo:omit($config, ., ("tei-death", css:map-rend-to-class(.)), .)
                    case element(idno) return
                        if (@source='treccani') then
                            fo:link($config, ., ("tei-idno1", css:map-rend-to-class(.)), 'Treccani', ., map {})
                        else
                            if (@source='kima') then
                                fo:link($config, ., ("tei-idno2", css:map-rend-to-class(.)), 'KIMA', 'https://geo-kima.org/Places/Details?id=' || string() || '/', map {})
                            else
                                $config?apply($config, ./node())
                    case element(persName) return
                        if (ancestor::p) then
                            fo:alternate($config, ., ("tei-persName1", "persName", css:map-rend-to-class(.)), ., ., id(@key, doc("/db/apps/tei-publisher/data/playground/aux/Zacuto_People.xml")))
                        else
                            if (parent::person and @type='he') then
                                fo:omit($config, ., ("tei-persName2", css:map-rend-to-class(.)), .)
                            else
                                if (parent::person) then
                    ...
                    case element(person) return
                        if (parent::listPersons) then
                            fo:inline($config, ., ("tei-person", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(placeName) return
                        fo:inline($config, ., ("tei-placeName", css:map-rend-to-class(.)), .)
                    case element(sex) return
                        fo:omit($config, ., ("tei-sex", css:map-rend-to-class(.)), .)
                    ...
                    case element(place) return
                        fo:inline($config, ., ("tei-place", css:map-rend-to-class(.)), .)
                    case element() return
