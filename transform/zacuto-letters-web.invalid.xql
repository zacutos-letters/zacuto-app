            ...
            return
                    ...
                    case element(bibl) return
                        if (parent::person) then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-bibl1", css:map-rend-to-class(.)), $content)
                        else
                    ...
                    case element(idno) return
                        if (@source='isni') then
                            ...
                            return
                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-idno1", css:map-rend-to-class(.)), $content)
                        else
                            if (@source='gnd') then
                                let $params := 
                                    map {
                    ...
                    case element(persName) return
                        if (@type='main' and @xml:lang='he') then
                            html:block($config, ., ("tei-persName1", css:map-rend-to-class(.)), .)
                        else
                            if (@type='main' and @xml:lang='en' and $parameters?view='biogram') then
                                html:block($config, ., ("tei-persName2", css:map-rend-to-class(.)), .)
                            else
                                if (ancestor::p) then
                                    (: FEHLER BEI WEBDARSTELLUNG! :)
                                    html:alternate($config, ., ("tei-persName3", "persName", css:map-rend-to-class(.)), ., ., id(@key, doc("/db/apps/zacutos-letters/data/people.xml"))/tei:TEI/standOff/listPerson/person, map {})
                                else
                            ...
                            if ($parameters?view='biogram') then
                                (
                                    ...
                                    if (exists(note[@type="bioSummary"])) then
                                        ...
                                        (),
                                    if (exists(note[@type="bioSummary"])) then
                                        html:block($config, ., ("tei-person12", css:map-rend-to-class(.)), note[@type="bioSummary"])
                                    else
                                        (),
                                    if (exists(idno)) then
                                        ...
                                        (),
                                    if (exists(idno)) then
                                        html:block($config, ., ("tei-person14", css:map-rend-to-class(.)), idno)
                                    else
                                        ()
                                )
                    ...
                    case element(placeName) return
                        if (ancestor::p) then
                            html:alternate($config, ., ("tei-placeName", "placeName", css:map-rend-to-class(.)), ., ., id(@key, doc("/db/apps/zacutos-letters/data/places.xml")), map {})
                        else
                            $config?apply($config, ./node())
