...
module namespace model="http://www.tei-c.org/pm/models/places/latex";
    ...
    $input !         (
            ...
            return
                        ...
                        if (@type='verse') then
                            ...
                            if (@type='title_page') then
                                ...
                                if (parent::body or parent::front or parent::back) then
                                    latex:section($config, ., ("tei-div3", css:map-rend-to-class(.)), .)
                                else
                    ...
                    case element(title) return
                        latex:heading($config, ., ("tei-title1", css:map-rend-to-class(.)), ., 2)
                    ...
                    case element(body) return
                        (
                            latex:block($config, ., ("tei-body1", css:map-rend-to-class(.)), .),
                            latex:index($config, ., ("tei-body2", css:map-rend-to-class(.)), ., 'toc')
                        )
                    ...
                    case element(birth) return
                        latex:omit($config, ., ("tei-birth", css:map-rend-to-class(.)), .)
                    case element(death) return
                        latex:omit($config, ., ("tei-death", css:map-rend-to-class(.)), .)
                    case element(idno) return
                        if (@source='treccani') then
                            latex:link($config, ., ("tei-idno1", css:map-rend-to-class(.)), 'Treccani', ., map {})
                        else
                            if (@source='kima') then
                                latex:link($config, ., ("tei-idno2", css:map-rend-to-class(.)), 'KIMA', 'https://geo-kima.org/Places/Details?id=' || string() || '/', map {})
                            else
                                $config?apply($config, ./node())
                    case element(persName) return
                        if (ancestor::p) then
                            latex:alternate($config, ., ("tei-persName1", "persName", css:map-rend-to-class(.)), ., ., id(@key, doc("/db/apps/tei-publisher/data/playground/aux/Zacuto_People.xml")))
                        else
                            if (parent::person and @type='he') then
                                latex:omit($config, ., ("tei-persName2", css:map-rend-to-class(.)), .)
                            else
                                if (parent::person) then
                    ...
                    case element(person) return
                        if (parent::listPersons) then
                            latex:inline($config, ., ("tei-person", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(placeName) return
                        latex:inline($config, ., ("tei-placeName", css:map-rend-to-class(.)), .)
                    case element(sex) return
                        latex:omit($config, ., ("tei-sex", css:map-rend-to-class(.)), .)
                    ...
                    case element(place) return
                        latex:inline($config, ., ("tei-place", css:map-rend-to-class(.)), .)
                    case element() return
