...
module namespace model="http://www.tei-c.org/pm/models/zacuto-sources/epub";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(note) return
                        if (@place) then
                            epub:note($config, ., ("tei-note1", css:map-rend-to-class(.)), ., @place, @n)
                        else
                            if (parent::div and not(@place)) then
                                epub:block($config, ., ("tei-note2", css:map-rend-to-class(.)), .)
                            else
                                if (not(@place)) then
