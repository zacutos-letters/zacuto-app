(:~
    ...
    ODD: /db/apps/zacutos-letters/resources/odd/places.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/places/web";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(pb) return
                        html:break($config, ., css:get-rendition(., ("tei-pb", css:map-rend-to-class(.))), ., 'page', (concat(if(@n) then concat(@n,' ') else '',if(@facs) then                   concat('@',@facs) else '')))
                        ...
                        if (@type='verse') then
                            ...
                            if (@type='title_page') then
                                ...
                                if (parent::body or parent::front or parent::back) then
                                    ...
                                    html:block($config, ., ("tei-div4", css:map-rend-to-class(.)), .)
                    ...
                    case element(title) return
                        html:heading($config, ., ("tei-title1", css:map-rend-to-class(.)), ., 2)
                    ...
                    case element(body) return
                        (
                            html:block($config, ., ("tei-body1", css:map-rend-to-class(.)), .),
                            html:index($config, ., ("tei-body2", css:map-rend-to-class(.)), 'toc', .)
                        )
                    ...
                    case element(birth) return
                        html:omit($config, ., ("tei-birth", css:map-rend-to-class(.)), .)
                    case element(death) return
                        html:omit($config, ., ("tei-death", css:map-rend-to-class(.)), .)
                    case element(idno) return
                        if (@source='treccani') then
                            html:link($config, ., ("tei-idno1", css:map-rend-to-class(.)), 'Treccani', ., (), map {})
                        else
                            if (@source='kima') then
                                html:link($config, ., ("tei-idno2", css:map-rend-to-class(.)), 'KIMA', 'https://geo-kima.org/Places/Details?id=' || string() || '/', (), map {})
                            else
                                $config?apply($config, ./node())
                    case element(persName) return
                        if (ancestor::p) then
                            html:alternate($config, ., ("tei-persName1", "persName", css:map-rend-to-class(.)), ., ., id(@key, doc("/db/apps/tei-publisher/data/playground/aux/Zacuto_People.xml")), map {})
                        else
                            if (parent::person and @type='he') then
                                html:omit($config, ., ("tei-persName2", css:map-rend-to-class(.)), .)
                            else
                                if (parent::person) then
                    ...
                    case element(person) return
                        if (parent::listPersons) then
                            html:inline($config, ., ("tei-person", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(placeName) return
                        html:inline($config, ., ("tei-placeName", css:map-rend-to-class(.)), .)
                    case element(sex) return
                        html:omit($config, ., ("tei-sex", css:map-rend-to-class(.)), .)
                    ...
                    case element(geo) return
                        (
                            ...
                            html:webcomponent($config, ., ("tei-geo2", css:map-rend-to-class(.)), ., 'pb-geolocation', map {"longitude": tokenize(., ' ')[2], "latitude": tokenize(., ' ')[1], "emit": 'place'}),
                            html:inline($config, ., ("tei-geo3", css:map-rend-to-class(.)), .)
                        )

                    case element(place) return
                        html:inline($config, ., ("tei-place", css:map-rend-to-class(.)), .)
