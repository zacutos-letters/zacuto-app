...
declare function templates:form-control($node as node(), $model as map(*)) as node()* {
    ...
    let $log := util:log("info", "$control: "|| $control)
    ...
    return
        ...
        case "select" return
            ...
            let $log := util:log("info", "value: "|| $value)
            return
                element { node-name($node) } {
                    $node/@*,
        ...
        case "paper-checkbox" return            
            let $name := $node/@name
            let $cb-value := $node/@value
            let $value := templates:get-configuration($model, "templates:form-control")($templates:CONFIG_PARAM_RESOLVER)($name)
            let $log := util:log("info", "value: " || serialize($value))
            return
                if(contains($value, $cb-value)) then ( 
                    element { node-name($node) } {
                        $node/@* except $node/@checked,
                        attribute checked { "checked" },
                        $node/node()
        ...
        case "pb-select" return                            
            let $name := $node/@name
            let $cb-values := $node/@values
            let $values := templates:get-configuration($model, "templates:form-control")($templates:CONFIG_PARAM_RESOLVER)($name)
            let $log := util:log("info", "value: " || serialize($values))
            let $pb-value := if(empty($values)) then () else (
                let $tmp := for $val in $values 
                    return 
                        if($val = "null") then () 
                        else "&#34;" || $val || "&#34;"
                return
            ...
            return 
                if(empty($pb-value)) then ( 
                    element { node-name($node) } {
                        $node/@* except $node/@values,
                        $node/node()
                    }
                ) else ( 
                    element { node-name($node) } {
                        $node/@* except $node/@values,
                        attribute values { $pb-value },
                        $node/node()
                    }
                )                     
        default return
